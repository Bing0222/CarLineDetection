...
def train_data_generator(image_root, image_list, label_root, label_list, batch_size, out_size, height_crop_offset):
    ...
    while True:
        ...
        for i in indices:
            try:
                image = cv2.imread(os.join(image_root, image_list[i]))
                label = cv2.imread(os.join(label_root, label_list[i]), cv2.IMREAD_GRAYSCALE)
            except:
                continue
...
def decodePredicts2(predicts: torch.Tensor, out_size, height_pad_offset, mode='gray'):
    ...
    if mode == 'color':
        ...
        raise ValueError('mode supports:color/gray')
