...
class ConfigTrain(object):
    ...
    DATA_LIST_ROOT = os.join(PROJECT_ROOT, 'data_list')
    TRAIN_ROOT = '/root/data/LaneSeg'
    IMAGE_ROOT = os.join(TRAIN_ROOT, 'Image_Data')
    LABEL_ROOT = os.join(TRAIN_ROOT, 'Gray_Label')
    WEIGHTS_ROOT = os.join(PROJECT_ROOT, 'weights')
    WEIGHTS_SAVE_ROOT = os.join(WEIGHTS_ROOT, '1536x512_b2')
    LOG_ROOT = os.join(PROJECT_ROOT, 'logs')

    LOG_SUSPICIOUS_FILES = os.join(LOG_ROOT, 'suspicious_files.log')
    ...
    PRETRAINED_WEIGHTS = os.join(WEIGHTS_ROOT, '1024x384_b4', 'weights_ep_21_0.008_0.013_0.648.pth')
    BASE_LR = 0.001  # 学习率
    LR_STRATEGY = [
        [0.001], # epoch 0
        [0.001], # epoch 1
...
class ConfigInference(object):
    ...
    IMAGE_ROOT = os.join(DATA_ROOT, 'TestImage')
    LABEL_ROOT = os.join(DATA_ROOT, 'TestLabel')
    OVERLAY_ROOT = os.join(DATA_ROOT, 'TestOverlay')
    WEIGHTS_ROOT = os.join(PROJECT_ROOT, 'weights')
    PRETRAINED_WEIGHTS = os.join(WEIGHTS_ROOT, '1536x512_b2', 'weights_ep_18_0.007_0.011_0.684.pth')
    LOG_ROOT = os.join(PROJECT_ROOT, 'logs')
...
class ConfigVal(object):
    ...
    IMAGE_ROOT = os.join(VAL_ROOT, 'Image_Data')
    LABEL_ROOT = os.join(VAL_ROOT, 'Gray_Label')
    WEIGHTS_ROOT = os.join(PROJECT_ROOT, 'weights')
    VAL_WEIGHTS = os.join(WEIGHTS_ROOT, '1536x512_b2', 'weights_ep_18_0.007_0.011_0.684.pth')
    LOG_ROOT = os.join(PROJECT_ROOT, 'logs')
