    ...
    while True:
        ...
        if cfg.LABEL_MODE == 'gray':
            for i in range(outs.shape[0]):
                cv2.imwrite(join(cfg.LABEL_ROOT, images_filename[i].replace('.jpg', '_bin.png')), outs[i])
        elif cfg.LABEL_MODE == 'color':
            outs = outs[:, :, :, ::-1]
            for i in range(outs.shape[0]):
                # cv2.imwrite(pjoin(cfg.LABEL_ROOT, images_filename[i].replace('.jpg', '_bin_2.png')), outs[i])
                org_image = cv2.imread(join(cfg.IMAGE_ROOT, images_filename[i]))
                overlay_image = cv2.addWeighted(org_image, 0.6, outs[i], 0.4, gamma=0)
                cv2.imwrite(join(cfg.OVERLAY_ROOT, images_filename[i].replace('.jpg', '.png')), overlay_image)
