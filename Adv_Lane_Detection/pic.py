...
import numpy as np
from correction import distort_correct
...
camera = pickle.load(open( "Adv_Lane_Detection/camera_matrix.pkl", "rb" ))
mtx = camera['mtx']
dist = camera['dist']
camera_img_size = camera['imagesize']

# 矫正相机畸变
image = mpimg.imread('Adv_Lane_Detection/test_images/test1.jpg')
...
def binary_pipeline(img):
    ...
    # color channels
    # 对HLS颜色空间中的S通道和L通道进行阈值处理，
    # 获取饱和度和亮度信息，其中S通道阈值范围为(140, 255)，L通道阈值范围为(120, 255)
    s_binary = hls_select(img_copy, sthresh=(140, 255), lthresh=(120, 255))
    #red_binary = red_select(img_copy, thresh=(200,255))
    
    # Sobel x
    # 对图像的x方向和y方向进行Sobel梯度计算，
    # 然后对梯度结果进行二值化处理，获取x方向和y方向的梯度信息
    ...
    #magnitude & direction
    # 对图像的梯度幅值和梯度方向进行阈值处理，获取梯度幅值和梯度方向信息
    ...
    # Stack each channel
    # 结果进行合并，得到最终的二值化图像。
    # 具体实现为，将x方向和y方向的梯度信息和梯度幅值和梯度方向的信息进行逻辑“与”运算，
    # 并将结果与HLS颜色通道的阈值处理结果进行逻辑“或”运算，
    # 得到最终的二值化图像
...
plt.tight_layout()
plt.savefig('Adv_Lane_Detection/out_images/combined_filters.png')


image = mpimg.imread('Adv_Lane_Detection/test_images/test5.jpg')
image = distort_correct(image,mtx,dist,camera_img_size)
plt.imshow(image)
# plt.show()
...
def warp_image(img):
    ...
    warp_img = cv2.warpPerspective(img,perspective_transform,image_size,flags=cv2.INTER_LINEAR)


    return warp_img, inverse_perspective_transform
...
draw_poly = cv2.polylines(image,[source_points],True,(255,0,0), 5)
...
plt.subplots_adjust(left=0., right=1, top=0.9, bottom=0.)
plt.tight_layout()
plt.savefig('Adv_Lane_Detection/out_images/perspective_transform.png')
# plt.show()

image = mpimg.imread('Adv_Lane_Detection/test_images/test5.jpg')
image = distort_correct(image,mtx,dist,camera_img_size)

hist = np.sum(birdseye_result[int(birdseye_result.shape[0]/2):,:], axis=0)
plt.figure()
plt.plot(hist)
plt.savefig('Adv_Lane_Detection/out_images/lane_histogram.png')
plt.show()
...
5. Identify all of the non-zero pixels in that box. 
If there are enough, center the box on their mean position for the next window. 
